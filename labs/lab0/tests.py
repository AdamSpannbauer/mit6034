...
def count_pattern_3_getargs():
    ...
    answers["count_pattern_3_random"] = [
        [1, 2, 3, 2, 3],
...
count_pattern_3_expected = (
    "an integer between 1 and 10 (this test is randomly generated)"
)
...
def distribution_5_getargs():
    answers["distribution_5_random"] = [
        encode_sumprod(
            Sum(
                [
                    2 * random.randint(1, 50),
                    Product([3, Product([8, Sum(["x", "y"]), 5])]),
                ]
            )
        )
    ]
...
def distribution_5_testanswer(ans, original_val=None):
    ...
    return evaluator(ans, context) == evaluator(original_val[0], context) and is_flat(
        ans
    )
